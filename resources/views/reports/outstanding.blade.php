...
<div class="main-content">
    ...
    <div class="row g-3 mb-3">
        <div class="col-md-4"><div class="card border-0 shadow-sm p-3 text-center"><div class="text-muted">Total Billed</div><h4 class="text-primary">Rs. {{ number_format($totalAmount, 0) }}</h4></div></div>
        <div class="col-md-4"><div class="card border-0 shadow-sm p-3 text-center"><div class="text-muted">Total Paid</div><h4 class="text-success">Rs. {{ number_format($totalPaid, 0) }}</h4></div></div>
        <div class="col-md-4"><div class="card border-0 shadow-sm p-3 text-center"><div class="text-muted">Total Outstanding</div><h4 class="text-danger">Rs. {{ number_format($totalDue, 0) }}</h4></div></div>
    </div>
        ...
        <div class="card-body p-0">
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    @php $paid = $voucher->receipts->sum('amount'); $balance = $voucher->net_amount - $paid; @endphp
                    <tr>
                        <td>{{ $voucher->voucher_no }}</td>
                        <td>{{ $voucher->client->name }}<br><small class="text-muted">{{ $voucher->client->case_number ?? '' }}</small></td>
                        <td>{{ $voucher->voucher_date }}</td>
                        <td>{{ $voucher->taxYear->tax_year ?? '-' }}</td>
                        <td class="text-end">Rs. {{ number_format($voucher->net_amount, 0) }}</td>
                        <td class="text-end text-success">Rs. {{ number_format($paid, 0) }}</td>
                        <td class="text-end text-danger"><strong>Rs. {{ number_format($balance, 0) }}</strong></td>
                        <td><span class="badge bg-{{ $voucher->status == 'Partial' ? 'warning' : 'danger' }}">{{ $voucher->status }}</span></td>
                    </tr>
                    @empty
                ...
                <tfoot class="table-dark">
                    <tr><td colspan="4"><strong>Total</strong></td><td class="text-end"><strong>Rs. {{ number_format($totalAmount, 0) }}</strong></td><td class="text-end"><strong>Rs. {{ number_format($totalPaid, 0) }}</strong></td><td class="text-end"><strong>Rs. {{ number_format($totalDue, 0) }}</strong></td><td></td></tr>
                </tfoot>
            </table>
        </div>
    </div>
