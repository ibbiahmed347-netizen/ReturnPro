...
<head>
    ...
    <title>Income Tax Returns - ReturnPro</title>
...
<div class="main-content">
    <div class="topbar">
        <h5 class="mb-0">Income Tax Returns</h5>
    ...
    <div class="card border-0 shadow-sm mb-3">
        <div class="card-body">
            <form method="GET" class="row g-2">
                <div class="col-md-4">
                    <input type="text" name="search" class="form-control" placeholder="Search by client name, case no, NTN..." value="{{ request('search') }}">
                </div>
                <div class="col-md-2">
                    <select name="tax_year_id" class="form-select">
                        <option value="">All Years</option>
                        @foreach($taxYears as $year)
                        <option value="{{ $year->id }}" {{ request('tax_year_id') == $year->id ? 'selected' : '' }}>{{ $year->tax_year }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-2">
                ...
                <div class="col-md-2">
                    <a href="{{ route('income-tax.create') }}" class="btn btn-success w-100"><i class="fas fa-plus"></i> Add Return</a>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="card-body p-0">
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><a href="{{ route('clients.show', $return->client) }}" class="text-decoration-none fw-semibold">{{ $return->client->name }}</a></td>
                        <td>{{ $return->client->case_number ?? '-' }}</td>
                        <td>{{ $return->client->ntn ?? '-' }}</td>
                        <td><span class="badge bg-primary">{{ $return->taxYear->tax_year }}</span></td>
                        <td><span class="badge bg-secondary">{{ $return->return_type }}</span></td>
                        <td><span class="badge bg-{{ $return->return_status == 'Published' ? 'success' : 'warning' }}">{{ $return->return_status }}</span></td>
                        <td>{{ $return->created_at->format('d-m-Y') }}</td>
                        <td>
                            <a href="{{ route('income-tax.show', $return) }}" class="btn btn-sm btn-info text-white"><i class="fas fa-eye"></i></a>
                            @if($return->return_status == 'Draft')
                            <a href="{{ route('income-tax.edit', $return) }}" class="btn btn-sm btn-warning text-white"><i class="fas fa-edit"></i></a>
                            <form method="POST" action="{{ route('income-tax.destroy', $return) }}" class="d-inline" onsubmit="return confirm('Delete this return?')">
                                @csrf @method('DELETE')
                    ...
                    @empty
                    <tr><td colspan="8" class="text-center text-muted py-4"><i class="fas fa-file-alt fa-2x mb-2 d-block"></i>No returns found. <a href="{{ route('income-tax.create') }}">Add first return</a></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
