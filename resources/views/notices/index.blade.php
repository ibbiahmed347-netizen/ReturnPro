...
<div class="main-content">
    <div class="topbar">
        <h5 class="mb-0">Notices Management</h5>
    ...
    <div class="card border-0 shadow-sm mb-3">
        <div class="card-body">
            <form method="GET" class="row g-2">
                <div class="col-md-5">
                    <input type="text" name="search" class="form-control" placeholder="Search by subject, notice no, client..." value="{{ request('search') }}">
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-2">
                    <a href="{{ route('notices.create') }}" class="btn btn-success w-100"><i class="fas fa-plus"></i> Add Notice</a>
                </div>
            </form>
        </div>
    </div>
            ...
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('clients.show', $notice->client) }}" class="text-decoration-none fw-semibold">{{ $notice->client->name }}</a>
                            <br><small class="text-muted">{{ $notice->client->case_number ?? '' }}</small>
                        </td>
                        <td>{{ $notice->subject }}</td>
                        <td>{{ $notice->notice_date }}</td>
                        <td><span class="badge bg-secondary">{{ $notice->replies->count() }}</span></td>
                        <td><span class="badge bg-{{ $notice->status_color }}">{{ $notice->status }}</span></td>
                        <td>
                            <a href="{{ route('notices.show', $notice) }}" class="btn btn-sm btn-info text-white"><i class="fas fa-eye"></i></a>
                            <a href="{{ route('notices.edit', $notice) }}" class="btn btn-sm btn-warning text-white"><i class="fas fa-edit"></i></a>
                            <form method="POST" action="{{ route('notices.destroy', $notice) }}" class="d-inline" onsubmit="return confirm('Delete this notice?')">
                                @csrf @method('DELETE')
                    ...
                    @empty
                    <tr><td colspan="7" class="text-center text-muted py-4"><i class="fas fa-bell fa-2x mb-2 d-block"></i>No notices found. <a href="{{ route('notices.create') }}">Add first notice</a></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if($notices->hasPages())<div class="card-footer">{{ $notices->appends(request()->query())->links() }}</div>@endif
    </div>
</div>
